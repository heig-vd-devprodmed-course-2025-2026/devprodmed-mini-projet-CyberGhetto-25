<x-default-layout>
    ...
    @if (session('plain_text_token'))
        <div class="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900 border border-yellow-400 dark:border-yellow-600 rounded-md">
            ...
            <code
                class="block break-all text-sm bg-white dark:bg-neutral-800 text-gray-900 dark:text-gray-100 p-2 rounded border border-yellow-300 dark:border-yellow-700">{{ session('plain_text_token') }}</code>
        </div>
    @endif
                            ...
                            <td class="py-3 pr-4">
                                <div class="flex flex-wrap gap-1">
                                    @foreach ($token->abilities as $ability)
                                        <span
                                            class="px-2 py-0.5 text-xs rounded bg-gray-100 dark:bg-neutral-700 text-gray-700 dark:text-gray-300">
                                            {{ $ability }}
                                        </span>
                                    @endforeach
                                </div>
